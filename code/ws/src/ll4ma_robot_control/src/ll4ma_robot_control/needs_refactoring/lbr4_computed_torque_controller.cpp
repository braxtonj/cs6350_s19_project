...
Eigen::VectorXd q_dot;// current joint velocity
vector<double> q_des;// desired joint position
...
int main(int argc, char** argv)
{
  ...
  vector<float> k_p={500.0,100.0,500.0,500.0,500.0,500.0,100.0};
  vector<float> k_d={0.01,0.1,0.01,0.01,0.01,0.01,0.01};  
  q_des.resize(7);
	  ...
	  if(got_new_cmd)
	  {
	      ...
	      for(int i=0;i<DOF;++i)
		{
		  q_diff=q_des[i]-q[i];
                  ...
                  {
                    q_acc[i]=0.0;
                    q_dot_des[i]=std::copysign(max_delta_q,q_diff);;
                    q_diff=std::copysign(max_delta_q,q_diff);
                  }
                  
                  if(std::abs(q_des[i]-q[i])<goal_thresh)
                  {
                    q_dot_des[i]=0.0;
                  }

		  tau_control=k_p[i]*(q_diff)+k_d[i]*(q_dot_des[i]-q_dot[i]);
                  if(std::abs(tau_control)>max_tau)
                  {
                    tau_control=std::copysign(max_tau,tau_control);
                  }
