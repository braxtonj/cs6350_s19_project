...
def jointstate_to_numpy(js_msg):
	return np.array(js_msg.position)
    
@converts_from_numpy(JointState)
def numpy_to_jointstate(js_array):
	new_js = JointState()
	for s in np.nditer( js_array ):
		new_js.position.append( s )
	return new_js
