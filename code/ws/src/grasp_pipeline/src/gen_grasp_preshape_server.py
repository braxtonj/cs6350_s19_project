...
class GenGraspPreshape():
    def __init__(self):
        ...
        #    rospy.Subscriber(gazebo_model_state_topic, ModelStates, self.obj_palm_pose_cb)
        self.listener = tf.TransformListener()
        self.tf_br = tf.TransformBroadcaster()
        self.service_is_called = False
        self.object_pose = PoseStamped()
        #self.palm_goal_pose_in_pcd = PoseStamped()
        #self.bb_faces_num = 3
        #self.palm_goal_pose_in_pcd = [PoseStamped()] * self.bb_faces_num
        self.palm_goal_pose_in_pcd = [] 
    ...
    def setup_joint_angle_limits(self):
        ...
        #Limit for data collection
        #self.first_joint_lower_limit = 0.1
        #self.first_joint_upper_limit = 0.1
        #self.second_joint_lower_limit = 0.5
        #self.second_joint_upper_limit = 0.
        ...
        #self.second_joint_lower_limit = 0.25
        #self.second_joint_upper_limit = 0.25

        #self.thumb_1st_joint_lower_limit = 0.25
        #self.thumb_1st_joint_upper_limit = 0.25
        #self.thumb_2nd_joint_lower_limit = 0.25
        #self.thumb_2nd_joint_upper_limit = 0.25
        ...
        self.first_joint_upper_limit = 0.1
        self.second_joint_lower_limit = 0.5
        self.second_joint_upper_limit = -0.1
    ...
    def gen_grasp_preshape(self, req):
        ...
        #print bb_center_points, bb_faces
        self.palm_goal_pose_in_pcd = []
        #for i in xrange(self.bb_faces_num):
        for i in xrange(len(bb_faces)):
            # Get desired palm pose given the point from the bounding box
            palm_pose_in_pcd = self.find_palm_pose(bb_center_points[i], req.obj, bb_faces[i])
            #self.palm_goal_pose_in_pcd[i] = palm_pose_in_pcd
            self.palm_goal_pose_in_pcd.append(palm_pose_in_pcd)
            response.palm_goal_pose_in_pcd.append(palm_pose_in_pcd)
    ...
    def sample_grasp_preshape(self, req):
        ...
        for i in xrange(len(bb_faces)):
            ...
            self.set_palm_rand_pose_limits(palm_pose_in_pcd)
            for j in xrange(self.samples_num_per_preshape):
                sample_palm_pose = self.sample_palm_rand_pose(palm_pose_in_pcd.header.frame_id)
                response.palm_goal_pose_in_pcd.append(sample_palm_pose)
                # Sample remaining joint values
                allegro_js = self.gen_allegro_random_preshape_js()
                response.allegro_joint_state.append(allegro_js) 
                response.is_top_grasp.append(bb_faces[i].is_top)
    ...
    def broadcast_palm_and_obj(self):
        ...
        if self.service_is_called:
            ...
            self.tf_br.sendTransform((self.object_pose.pose.position.x, self.object_pose.pose.position.y, 
                    ...
                    rospy.Time.now(), 'grasp_object', self.object_pose.header.frame_id)

            # Publish the palm goal tf
            for i, palm_pose_in_pcd in enumerate(self.palm_goal_pose_in_pcd):
                self.tf_br.sendTransform((palm_pose_in_pcd.pose.position.x, palm_pose_in_pcd.pose.position.y, 
                        palm_pose_in_pcd.pose.position.z),
                        (palm_pose_in_pcd.pose.orientation.x, palm_pose_in_pcd.pose.orientation.y, 
                        palm_pose_in_pcd.pose.orientation.z, palm_pose_in_pcd.pose.orientation.w),
                        rospy.Time.now(), 'heu_' + str(i), palm_pose_in_pcd.header.frame_id)
    ...
    def get_bb_faces(self, grasp_object):
        ...
        #face_centers_camera_frame = []
        #face_camera_depth_map = {}
        ...
        #    center_stamped_camera = self.listener.transformPoint('kinect2_rgb_optical_frame', center_stamped_world)
        #    face_centers_world_frame.append(copy.deepcopy(center_stamped_world))
        #    face_centers_camera_frame.append(center_stamped_camera)
        #    face_camera_depth_map[center_stamped_camera.point.z] = i

        #if self.use_sim:
        #    # z value of blensor point cloud is negative.
        #    face_camera_depth_map = sorted(face_camera_depth_map.items(), key=lambda kv: abs(kv[0]))
        #else:
        #    face_camera_depth_map = sorted(face_camera_depth_map.items(), key=lambda kv: kv[0])
        ## Delete the two farthest faces of the BB in terms of the camera z axis
        #del faces_world_frame[face_camera_depth_map[-1][1]]
        #del faces_world_frame[face_camera_depth_map[-2][1]]
        #del face_centers_world_frame[face_camera_depth_map[-1][1]]
        #del face_centers_world_frame[face_camera_depth_map[-2][1]]
        #del face_centers_camera_frame[face_camera_depth_map[-1][1]]
        #del face_centers_camera_frame[face_camera_depth_map[-2][1]]

        # Sort BB faces based on the x coordinates in world frame
        faces_world_frame = sorted(faces_world_frame, key=lambda x: x.center[0])
        # Delete these two faces with smallest x coordinates in world frame, 
        # since the camera has a postive x coord larger than any point on the table,
        # this is one way to delete faces hidden from the camera.
        faces_world_frame = faces_world_frame[:3]
        face_centers_world_frame = []
        center_stamped_world = PointStamped()
        center_stamped_world.header.frame_id = grasp_object.header.frame_id
        for i, face in enumerate(faces_world_frame):
        ...
        #publish_points(self.grasp_pose_pub, face_centers_camera_frame)
    ...
    def find_palm_pose(self, object_point_stamped, obj, bb_face):
        ...
        # Use normal and closest point to generate palm position
        #hand_dist = self.hand_sample_dist

        # Add Gaussian noise to the palm position
        if self.hand_dist_var > 0.0:
            hand_dist += np.random.normal(0.0, self.hand_dist_var)
    ...
    def create_preshape_server(self):
        ...
        '''
        #if self.use_sim:
        #    self.preshape_sim_service = rospy.Service('gen_grasp_preshape', GraspPreshape,
        #                                              self.handle_gen_grasp_preshape_sim)
        #else:
    ...
    def handle_update_detection_preshape(self, req):
        #self.palm_goal_pose_in_pcd = req.exp_palm_poses
    ...
    def obj_palm_pose_cb(self, gz_model_msg):
        self.obj_pose = PoseStamped()
        self.obj_pose.header.frame_id = 'world'
        self.obj_pose.pose = gz_model_msg.pose[2]
        #tf_br = tf.TransformBroadcaster()
        pos = self.obj_pose.pose.position
        ort = self.obj_pose.pose.orientation
        self.tf_br.sendTransform((pos.x, pos.y, pos.z), (ort.x, ort.y, ort.z, ort.w), rospy.Time.now(), 'grasp_object', 'world')
        self.tf_br.sendTransform((self.palm_pose.pose.position.x, self.palm_pose.pose.position.y, self.palm_pose.pose.position.z),
                (self.palm_pose.pose.orientation.x, self.palm_pose.pose.orientation.y, self.palm_pose.pose.orientation.z, self.palm_pose.pose.orientation.w),
                rospy.Time.now(), 'goal_palm', 'grasp_object')
