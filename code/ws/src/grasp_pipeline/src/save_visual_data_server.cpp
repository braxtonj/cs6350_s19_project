                ...
                grasp_pipeline::SaveVisualData::Response &res)
{
    ...
    //Binary
    //createDirectory(req.scene_cloud_save_path);
    ...
    // Convert HD cloud to rgb image and save it.
    //sensor_msgs::Image rgb_image_msg;
    //pcl::toROSMsg(req.scene_cloud, rgb_image_msg);
    ////sensor_msgs::ImageConstPtr rgb_image_msg_ptr = boost::shared_ptr<sensor_msgs::Image const>(rgb_image_msg) 
    ////sensor_msgs::ImageConstPtr rgb_image_msg_ptr(rgb_image_msg);
    //// Below are two ways to create a boost pointer, but why the upper two don't work?
    ////sensor_msgs::ImageConstPtr rgb_image_msg_ptr = boost::shared_ptr<sensor_msgs::Image const>(new sensor_msgs::Image(rgb_image_msg));
    //sensor_msgs::ImageConstPtr rgb_image_msg_ptr(new sensor_msgs::Image(rgb_image_msg));
    //cv_bridge::CvImagePtr cv_ptr;
    //cv_ptr = cv_bridge::toCvCopy(rgb_image_msg_ptr, sensor_msgs::image_encodings::BGR8);
    ////plotFingerTipPalmCenter(cv_ptr->image, req.preshape_finger_tip_image_locs_plot, req.preshape_palm_image_loc_plot);
    ////createDirectory(req.rgb_image_save_path);
    //imwrite(req.rgb_image_save_path, cv_ptr->image);

    Mat rgb_image;
    cv_bridge::CvImagePtr cv_rgb_msg = cv_bridge::toCvCopy(req.scene_rgb_img, sensor_msgs::image_encodings::BGR8);
    //plotFingerTipPalmCenter(cv_rgb_msg->image, req.preshape_finger_tip_image_locs_plot, req.preshape_palm_image_loc_plot);
    imwrite(req.rgb_image_save_path, cv_rgb_msg->image);

    //cv_rgb_msg->image.convertTo(rgb_image, CV_32FC3);
    //saveRgbImage(rgb_image, req.rgb_image_save_path);

    Mat depth_image;
    //getDepthFromPcd(raw_cloud_ptr, depth_image);
    ...
    // cv::waitKey();
    //createDirectory(req.depth_image_save_path);
    ...
    //createDirectory(req.scene_sd_cloud_save_path);
    pcl::io::savePCDFile(req.scene_sd_cloud_save_path, *raw_sd_cloud_ptr, true);

    //Mat sd_cloud_depth_image;
    //getDepthFromPcd(raw_sd_cloud_ptr, sd_cloud_depth_image);
    //saveDepthImage(sd_cloud_depth_image, req.sd_cloud_depth_image_save_path);

    //sensor_msgs::Image sd_cloud_rgb_image_msg;
    //pcl::toROSMsg(req.scene_sd_cloud, sd_cloud_rgb_image_msg);
    //sensor_msgs::ImageConstPtr sd_cloud_rgb_image_msg_ptr(new sensor_msgs::Image(sd_cloud_rgb_image_msg));
    //cv_ptr = cv_bridge::toCvCopy(sd_cloud_rgb_image_msg_ptr, sensor_msgs::image_encodings::BGR8);
    //imwrite(req.sd_cloud_rgb_image_save_path, cv_ptr->image);
    ...
    //createDirectory(req.sd_cloud_normal_save_path);
